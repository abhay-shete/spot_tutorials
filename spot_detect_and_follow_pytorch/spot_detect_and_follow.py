...
def start_detector_processes(num_processes, model_path, detection_class, detection_threshold,
...
def process_images(model_path, detection_class, detection_threshold, max_processing_delay):
    ...
    while not SHUTDOWN_FLAG.value:
        ...
        for _, capture in entry.items():
            ...
            boxes, scores, classes = odapi.predict(image, model)
            ...
            for box, score, box_class in sorted(zip(boxes, scores, classes), key=lambda x: x[1],
                ...
                if score > detection_threshold and box_class == detection_class:
                    ...
                    confident_object_classes.append(class_names_dict[box_class])
...
def main(argv):
    ...
    parser.add_argument('--detection-threshold', default=0.5, type=float,
                        help='Detection threshold to use for Tensorflow detections')
    parser.add_argument(
        '--sleep-between-capture', default=2, type=float,
    ...
    try:
        ...
        start_detector_processes(options.number_tensorflow_processes,
