...
os.environ["BOSDYN_CLIENT_PASSWORD"] = "Stonecrossbox"
...
def process_images(options, model_extension):
    ...
    while True:
        ...
        for i in range(len(boxes)):
            label = str(classes[i])
            # person, carrot, bottle
            if label.lower() not in ['1', '57', '44', '33']:
                continue
            box = boxes[i]
            score = scores[i]
...
def register_with_robot(options):
    ...
    kServiceName = 'fetch-server'
    kServiceAuthority = 'tensorflow-worker.spot.robot'

    sdk = bosdyn.client.create_standard_sdk('fetch_server')
...
def main(argv):
    ...
    parser.add_argument("--network", help="default")
    parser.add_argument("--ip", help="default")
